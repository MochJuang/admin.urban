                                        ...
                                        <div class="row">
                                            <div class="col-lg-12 col-xl-12">
                                                ...
                                                <ul class="nav nav-tabs data-tabs tabs" role="tablist">
                                                    <li class="nav-item"> <a class="nav-link kirim" data-toggle="tab" data-status="5" href="#kirim" role="tab">Kirim <span class="pcoded-badge label label-success transaksi" id="transaksi-5">0</span> </a></a> </li>
                                                    <li class="nav-item"> <a class="nav-link selesai" data-toggle="tab" data-status="6" href="#selesai" role="tab">Selesai <span class="pcoded-badge label label-success transaksi" id="transaksi-6">0</span> </a></a> </li>
                                                    <li class="nav-item"> <a class="nav-link expired" data-toggle="tab" data-status="7" href="#expired" role="tab">Expired <span class="pcoded-badge label label-success transaksi" id="transaksi-7">0</span> </a></a> </li>

                                                    <li class="nav-item"> <a class="nav-link cancel" data-toggle="tab" data-status="9" href="#cancel" role="tab">Cancel <span class="pcoded-badge label label-success transaksi" id="transaksi-9">0</span> </a></a> </li>
...
},
{
    ...
    var dataTable = $("#table-"+status).DataTable({
        "scrollX": true,
        "lengthChange": true,
        "dom": 'Bfrtip',
        "buttons": [
...
$('document').ready(function () {
    function checkFilter(e){
        ...
        if(startDate && lastDate){
            $('#saveFilter').removeClass('disabled')
        }
    }

    $(document).on('change', '#startDate',checkFilter)
    $(document).on('change', '#lastDate',checkFilter)

    $(document).on('click', '#saveFilter', function(e){
        if($(this).hasClass('disabled') || !$('#startDate').val() || !$('#lastDate').val())
        {
            return false;
            $('#saveFilter').addClass('disabled')
        }
        let status = null
        ...
        let query = '?transaksiStatus=' + status 
        query += '&startDate=' + $('#startDate').val()
        query += '&lastDate=' + $('#lastDate').val()

        tableName = null;

        $('.tab-content').find('div').each(function(k, v){
        ...
        $('#table-' + tableName).DataTable({
            ...
            "processing": true,
            'columns':columnsTransaksi
        }).ajax.url(base_url + 'transaksi/get' + query).load(); 

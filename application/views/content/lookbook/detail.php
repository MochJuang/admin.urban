                ...
                <form id="form-gambar">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            ...
                            <input id="data-excell" class="form-control" name="gambar" placeholder="Nama Artikel" type="file" required="required">
                        </div>
                    </div>
...
<script>
    ...
    $('document').ready(function() {
        ...
        $('#form-gambar').submit(function(e) {
            ...
            var url_form = (isEdit == false) ? base_url + '/lookbook/add_gambar' : base_url +
                '/lookbook/edit_gambar'
            $.ajax({
                url: url_form,
                type: 'post',
    ...
    function detail(params) {
        getMasterData(service_url + 'lookbook/' + params, '', function(data) {
            var gambar = ``;
            if (data.gambar) {
                var no = 0;
                data.gambar.forEach(e => {
                    if (e != '' && e != null) {
                        gambar += `<div data-id="${no}" class="col-lg-6 col-xl-3 col-md-6 card-sub">
                            <div class="card rounded-card user-card">
                                <div class="card-block">
                                    <div class="img-hover"> <img class="img-fluid img-radius" src="${encodeURI(path_photo+"lookbook/"+e)}" alt="round-img">
                ...
                var jum = data.gambar.filter(img => img != null && img != '');
    ...
    function delete_master_product(idData, field, endpoint) {
        swal({
            ...
            function() {
                var datanyas = ['gambar' + field, null];
                var datanya = {};
    ...
    Sortable.create(draggablePanelList, {
        ...
        store: {
            ...
            set: function(sortable) {
                ...
                getMasterData(service_url + 'lookbook/' + id_product, '', function(data) {
                    var no = 1;
                    var simpan = {
                        'gambar1': null,
                        'gambar2': null,
                    ...
                    for (ab in arr) {
                        simpan['gambar' + no] = data.gambar[arr[ab]];
                        no++;
                    }
                    $.ajax({
                        url: service_url + "lookbook/update/" + id_product,
                        type: 'PUT',
                        data: simpan,
                        dataType: 'json',
                        success: function(data, text) {
